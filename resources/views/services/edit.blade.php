                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                                  ...
                                  placeholder="Περιγράψτε λεπτομερώς την υπηρεσία σας...">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center">
                                <input type="radio" id="per_hour" name="rate_type" value="per_hour"
                                       {{ old('rate_type', $service->rate_type) === 'per_hour' ? 'checked' : '' }}
                                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                            ...
                            <div class="flex items-center">
                                <input type="radio" id="per_square_meter" name="rate_type" value="per_square_meter"
                                       {{ old('rate_type', $service->rate_type) === 'per_square_meter' ? 'checked' : '' }}
                                       class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300">
                    ...
                    <div id="rate_amount_section">
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            <div class="relative">
                                <input type="number" id="rate_amount" name="rate_amount"
                                       value="{{ old('rate_amount', $service->rate_amount) }}"
                                       step="0.01" min="0"
                                       class="w-full pl-4 pr-12 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('rate_amount') border-red-500 @enderror"
                                       placeholder="0.00">
                            ...
                            <select id="rate_currency" name="rate_currency"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('rate_currency') border-red-500 @enderror">
                                <option value="EUR" {{ old('rate_currency', $service->rate_currency) === 'EUR' ? 'selected' : '' }}>Ευρώ (€)</option>
                                <option value="USD" {{ old('rate_currency', $service->rate_currency) === 'USD' ? 'selected' : '' }}>Δολάριο ($)</option>
                            </select>
                        </div>
                        @error('rate_amount')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <div class="text-sm font-medium text-blue-800">Προεπισκόπηση τιμής</div>
                                    <div id="rate_preview" class="text-sm text-blue-700">{{ $service->formatted_rate }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                        ...
                        <button type="submit"
                                class="px-6 py-3 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            Ενημέρωση Υπηρεσίας
                        </button>
                    </div>
                </div>
